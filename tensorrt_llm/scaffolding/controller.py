...
from tensorrt_llm.scaffolding.task import (GenerationTask, RewardTask,
                                           ScaffoldingOutput, Task, ChatTask, MCPCallTask, MCPListTask)
import json
import time
...
class MCPController(Controller):
    ...
    def process_result(self, result: str):
        try:
            ...
            if "tool" in tool_call and "arguments" in tool_call:
                mcp_task = MCPTask.create_mcptask(tool_call['tool'], tool_call['arguments'], 1, 10)
                print(f"Executing tool: {tool_call['tool']}")
                print(f"With arguments: {tool_call['arguments']}")
                mcp_task.worker_tag = self.WorkerTag.MCP
                return [mcp_task]
    ...
    def process(self, tasks: List[Task], **kwargs):
        ...
        chattask = ChatTask.create_from_prompt(messages, tasks[0].input_str, available_tools)
        result_task = tasks[0]
        ...
        mcp_call_tasks = [MCPCallTask.create_mcptask(tool_call.function.name, tool_call.function.arguments)
        ...
        messages.append({"role": "assistant", "content": chattask.choice.message.content})
        finalchattask = ChatTask.create_from_prompt(messages, mcp_result, available_tools)
